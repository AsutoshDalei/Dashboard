...
func (s *Service) Send(toEmail, name, company, senderKey string, provider Provider) (string, error) {
	config, err := s.GetConfig(senderKey)
	if err != nil {
		return "", err
	}

	senderLabel, err := provider.Send(config.FromEmail, toEmail, name, company)
	if err != nil {
